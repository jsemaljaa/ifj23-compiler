...
int func_def() {
    ...
    generatorFuncDef1(item->key.str, item->data.func->argc, item->data.func); /////
    inFunc = true;
    RULE(func_body());
    inFunc = false;

    generatorFuncDef2(item->key.str); /////
...
int call_parameters_list() {
    ...
    if (token.type == TYPE_RPAR) {
        ...
        if (item->data.func->argc == 0) {
            generatorFuncCall(item->key.str, item->data.func->argc, &item->data.func->ret); /////
            GET_TOKEN_SKIP_EOL();
            return NO_ERRORS;
        } else {
            return SEMANTIC_CALL_RET_ERROR;
...
int if_statement() {
    ...
    GET_TOKEN_SKIP_EOL();
    generatorIfElse2(ifNumber); ///// 
    RULE(statement_list());
    generatorIfElse3(ifNumber); /////
    return NO_ERRORS;
}
