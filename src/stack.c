...
void Stack_Init( Stack *stack ) {
	...
	{
		stack->topIndex = -1; // nastavi hodnotu vrholu zasobniku na -1, abych podtvrdit ho prazdnost
		stack->array = (char*)malloc(STACK_SIZE * sizeof(char)); // alokujeme pamet pro ten array abych ne dostali segfault
		return;
	}
}
...
bool Stack_IsEmpty( const Stack *stack ) {
	return (stack->topIndex == -1); //pokud vyraz plati, vrati hodnotu 1, jinak vrati 0
}
...
bool Stack_IsFull( const Stack *stack ) {
	return (stack->topIndex == STACK_SIZE - 1); //vrati 1 pokud bude mit maximalni stack (v danem pripade to 20-1 = 19), jinak 0
}
...
void Stack_Top( const Stack *stack, char *dataPtr ) {
	if (Stack_IsEmpty(stack) == 1) {
        ...
        *dataPtr = stack->array[stack->topIndex]; //vkladame ukazatel na hodnotu v array abych vratilo hodnotu prvka
		return; 
    }
}
...
void Stack_Pop( Stack *stack ) {
	...
	{
		stack->array[stack->topIndex] = NULL; //smazeme hodnotu na poslednim miste
		stack->topIndex--; //zmensujeme index
		return;
	}
}
...
void Stack_Push( Stack *stack, char data ) {
	...
	{
		stack->topIndex++; //zvetsujeme index
		stack->array[stack->topIndex] = data; // vkladame hodnotu na misto zvetseniho indexu
		return;
	}
...
void Stack_Dispose(Stack *stack) {
    if (stack != NULL) { //overime je li neco v stacku
        stack->array = NULL; //uvolnim pole
		free(stack->array); //uvolnim pamet
        stack->topIndex = -1; // nastavime prazdnost stacku
    }
}
